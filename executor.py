...
def _enqueue_players(row: pandas.Series, players: Queue[tuple[int, str]], players_set: set[str]):
    players.put_nowait((row['id'], row['riot_id']))
    players_set.put(row['riot_id'])


def _initialize_players(session: Session, players: Queue[tuple[int, str]], players_set: set[str]):
    ...
    if len(df.index) > 0:
        df.apply(lambda row: _enqueue_players(row, players, players_set), axis=1)

        logger.info(f'Fetched {players.qsize()} players for search.')
        return
    ...
    raise Exception('No player candidates for search available in database!')
